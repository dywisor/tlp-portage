...
_NM_DIS="${_NM_DIS_DIR}/02_tlp-ifup"
...
#	# fixme: will the link be removed when tlp has been emerged
#	# with USE="-networkmanager" and the dispatcher script has been
#	# 'created' by eselect?
#	if use networkmanager; then
#		dodir "$_NM_DIS_DIR" || die
#		ln -s "${_LIB}/tlp-ifup" "${D}/${_NM_DIS}" || die
#	fi
...
pkg_postinst() {
	...
	if use networkmanager; then
		eerror "ignored use flag: networkmanager."
	fi
#	if ! use networkmanager; then
#		ewarn "The ifup script has been installed into ${_LIB}. 'eselect tlp net' may help you to set it up."
#	fi
